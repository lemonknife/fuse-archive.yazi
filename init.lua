-- NOTE: since personally using fuse-archive version >= 1.0
-- it is NO need to create folder manually
...
local function run_command(cmd, args)
	...
	local child, cmd_err =
		Command(cmd):args(args):cwd(cwd):stdin(Command.INHERIT):stdout(Command.PIPED):stderr(Command.PIPED):spawn()
